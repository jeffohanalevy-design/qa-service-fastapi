...
def ask(question: str = Query(..., description="Your natural-language question")):
    ...
    top_msgs = [m for m, sim in similarities[:3] if sim > 0.45]
    answer = summarize_answer(question, top_msgs)
...
def summarize_answer(question, messages):
    ...
    if is_date_related(question):
        ...
        unique_msgs = list({m["message"] for m in messages})
        if len(unique_msgs) == 1:
            return f"{user} mentioned: {unique_msgs[0]}"
        else:
            combined = " ".join(unique_msgs[:2])
            return f"{user} mentioned: {combined}"
